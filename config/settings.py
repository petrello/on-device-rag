...
class Settings(BaseSettings):
    ...
    EMBEDDING_DIM: int = Field(
        default=768,  # referred to: ibm-granite/granite-embedding-278m-multilingual
    ...
    HF_HOME: Path = Field(default=Path("embeddings"), description="Hugging Face cache directory")
    ...
    LLM_CONTEXT_WINDOW: int = Field(default=8192, ge=512, le=16384)
    LLM_THREADS: int = Field(default=4, ge=1, le=32)
    LLM_BATCH_SIZE: int = Field(default=512, ge=32, le=2048)
    ...
    USE_HIERARCHICAL_CHUNKING: bool = Field(default=False)
    ...
    def validate_model_path(cls, v):
        ...
        path = Path(v)
        if not path.exists():
            logging.error("Model file not found: %s", v)
